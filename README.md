<img width="467" height="918" alt="image" src="https://github.com/user-attachments/assets/f47c6fa4-b173-4ca6-8f51-84baf1cc4c53" />
NZB Beamer

A self-hosted Node.js/Express web app for beaming .nzb files straight to a Windows machine running NZBGet — and for managing the media library that grows around it. Built as a small dark-themed single-page app with no external dependencies beyond a browser, plus optional deep integration with Home Assistant via MQTT.

What it does

Drop an .nzb file into the web UI (from any device on the network) and NZB Beamer:

Parses it and shows size, file count and usenet groups
Saves it into a watched folder so NZBGet can pick it up
Tracks the real download progress by polling NZBGet's JSON-RPC API
Automatically moves the finished download into the right media folder
Deletes the processed .nzb file once the download is done
Reports all of the above — plus full server/hardware telemetry — to Home Assistant
Features
...
Requirements
Node.js
A running NZBGet instance on the same machine (or reachable over the network)
